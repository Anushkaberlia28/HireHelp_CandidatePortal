...
export class InterviewStatusController {
  async list(req: Request, res: Response) {
    try {
      ...
      const interviews = await interviewStatusService.list(req.user.id);
  ...
  async getById(req: Request, res: Response) {
    try {
      ...
      const { id } = req.params;
      const interview = await interviewStatusService.getById(parseInt(id), req.user.id);
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      const interview = await interviewStatusService.update(parseInt(id), req.user.id, data);
      res.status(200).json(interview);
    } catch (error) {
      res.status(500).json({ error: (error as Error).message });
    }
